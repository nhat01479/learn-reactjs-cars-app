	...
	const handleCreate = async (data) => {
		...
		if (uploadedImg?.url) {
			...
			if (createRes && createRes.data) {
				...
				closeModal();
				navigate("/");
				setCarList([...carList, createRes.data]);
				reset();
				setSelectImg({
					file: null,
					tempUrl: "",
			...
			toast.error("Vui lòng upload ảnh", { autoClose: 1000 });
		}
	};
	...
	const handleUploadImage = async () => {
		if (selectImg.file) {
			...
			if (uploadResult?.data && Object.keys(uploadResult?.data).length) {
				...
				toast.info("Upload ảnh thành công", {
					position: "top-right",
					autoClose: 2000,
				});
				setUploading(false);
